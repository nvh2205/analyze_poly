  ...
  implements OnModuleDestroy, OnApplicationBootstrap
{
  ...
  async onApplicationBootstrap() {
    this.logger.log('Restoring subscriptions from Redis...');
    try {
      ...
      if (existingTokens && existingTokens.length > 0) {
        ...
        for (const tokenId of existingTokens) {
          try {
            ...
            const startTimeMs = metadata?.startTimeMs;

            if (typeof startTimeMs === 'number' && startTimeMs > now) {
        ...
        if (tokensToRemove.length > 0) {
          await this.redisService.srem('active_clob_tokens', ...tokensToRemove);
          for (const tokenId of tokensToRemove) {
            this.subscribedTokens.delete(tokenId);
            await this.redisService.del(`token_metadata:${tokenId}`);
          }
          this.logger.log(
            `Removed ${tokensToRemove.length} tokens from Redis due to startTime gate / missing metadata`,
          );
        }

        if (tokensToSubscribe.length > 0) {
          this.logger.log(
            `Found ${tokensToSubscribe.length} tokens eligible by startTime in Redis, subscribing...`,
          );
          await this.subscribeToTokens(tokensToSubscribe);
        } else {
          this.logger.log(
            'No eligible tokens to subscribe after startTime gate',
          );
        }
      } else {
        this.logger.log('No existing tokens found in Redis');
      }
    } catch (error) {
      this.logger.error('Error restoring subscriptions from Redis:', error);
    }
  }
