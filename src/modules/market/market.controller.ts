...
export class MarketController {
  ...
  constructor(
    ...
    private readonly priceRangeAnalyticsService: PriceRangeAnalyticsService,
  ) {}
  ...
  @Get('price-ranges')
  @ApiOperation({
    summary: 'Get price range analytics for a market type',
    description:
      'Returns comprehensive price range analysis including auto-discovered ranges, ' +
      'range statistics (entries, dwell, hold, breakout), first-passage probabilities, ' +
      'and top edge setups. Data is cached in Redis and generated by offline Python analytics job.',
  })
  @ApiQuery({
    name: 'type',
    required: true,
    description: 'Market type (e.g., btc-updown-15m, eth-updown-15m)',
    example: 'btc-updown-15m',
  })
  @ApiQuery({
    name: 'tokenSide',
    required: false,
    description: 'Token side to analyze',
    example: 'yes',
  })
  @ApiQuery({
    name: 'startTime',
    required: false,
    description: 'Start time (Unix timestamp) for specific window',
  })
  @ApiQuery({
    name: 'endTime',
    required: false,
    description: 'End time (Unix timestamp) for specific window',
  })
  @ApiOkResponse({
    description: 'Price range analytics payload',
    schema: {
      type: 'object',
      properties: {
        metadata: {
          ...
          properties: {
            ...
            token_side: { type: 'string' },
            analysis_start: { type: 'string' },
            analysis_end: { type: 'string' },
        ...
        range_stats: { type: 'array' },
        first_passage: { type: 'array' },
        top_edges: { type: 'array' },
        edge_summary: {
          type: 'object',
          properties: {
            total_edges: { type: 'number' },
            avg_p_target: { type: 'number' },
            avg_ev: { type: 'number' },
            avg_reward_risk: { type: 'number' },
          },
        },
      },
    },
  })
  async getPriceRangeAnalytics(
    ...
    const analytics =
      await this.priceRangeAnalyticsService.getAnalytics({
        type,
        tokenSide: (tokenSide as 'yes' | 'no') || 'yes',
        startTime: startTime ? parseInt(startTime) : undefined,
        endTime: endTime ? parseInt(endTime) : undefined,
      });
    ...
    if (!analytics) {
      throw new BadRequestException(
        `No analytics found for type: ${type}. Run the Python analytics job first.`,
      );
    }
  ...
  @ApiOperation({
    summary: 'List market types with available analytics',
    description: 'Returns list of market types that have cached analytics',
  })
  @ApiOkResponse({
    description: 'Available market types',
    schema: {
      type: 'object',
      properties: {
        count: { type: 'number' },
  ...
  async getAvailableAnalyticsTypes() {
    const types = await this.priceRangeAnalyticsService.listAvailableTypes();
    return {
      count: types.length,
      types,
    };
